...
void WopiStorage::createCopyFile(const std::string& accessToken, const std::string& newFileName)
{
    ...
    try
    {
        ...
        request.set("X-WOPI-SuggestedTarget", newFileName + "." + getFileExtension());
        ...
        LOG_INF("WOPI::createCopyFile tried to create a copy of file at [" << uriObject.toString()
                << "] having a size of " << size << " bytes " << ", with the response "
