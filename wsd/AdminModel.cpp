...
void Document::takeSnapshot()
{
    ...
    auto pair = std::make_pair(now,std::make_shared<DocumentSnapshot>(*this));
    this->_snapshots.emplace(pair);
}

const std::map<std::time_t,std::shared_ptr<DocumentSnapshot>> Document::getHistory() const
{
    return std::map<std::time_t,std::shared_ptr<DocumentSnapshot>>(_snapshots);
}
...
AdminModel::~AdminModel()
{
    ...
    for (auto ed : _expiredDocuments)
    {
        ...
        {
            std::string theTime = ctime(&s.first);
            theTime.erase(theTime.find('\n', 0), 1);
            oss << " @ " << theTime << " -> " << s.second->to_string() << "\n";
        }
        oss << "\n";
    }
    Log::debug(oss.str());
