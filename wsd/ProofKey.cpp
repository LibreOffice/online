                                               ...
                                               const std::vector<unsigned char>& exponent)
{
    ...
    // make sure exponent length is correct; assume we are passed little-endian vectors
    std::vector<unsigned char> exponent4bytes(4);
    std::copy(exponent.begin(), exponent.end(), exponent4bytes.begin());
    ...
    std::copy(exponent4bytes.begin(), exponent4bytes.end(), std::back_inserter(capiBlob));
