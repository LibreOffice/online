...
VecOfStringPairs Proof::GetProofHeaders(const std::string& access_token, const std::string& uri) const
{
    ...
    if (m_pOldKey)
    {
        ...
        const auto aProof = GetProof(access_token, uri, ticks);
        vec.emplace_back("X-WOPI-ProofOld", SignProof(aProof, *m_pOldKey));
        vec.emplace_back("X-WOPI-Proof", SignProof(aProof, *m_pNewKey));
    }
    return vec;
}
