...
bool isPamAuthOk(const std::string user, const std::string pass)
{
    ...
    std::vector<char> userBuffer(user.length() + 1, '\0');
    std::copy(user.begin(), user.end(), userBuffer.begin());
    std::vector<char> passBuffer(pass.length() + 1, '\0');
    std::copy(pass.begin(), pass.end(), passBuffer.begin());

    retval = pam_start("loolwsd", &userBuffer[0], &localConversation, &localAuthHandle);
                                               ...
                                               HTTPResponse &response)
{
    ...
    if (credentials.getUsername() == user && userProvidedPwd == pass)
    {
        authenticated = true;
    }
    ...
    {
        authenticated = isPamAuthOk("timar", userProvidedPwd);
        //authenticated = true;
    }
