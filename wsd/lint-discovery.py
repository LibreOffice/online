...
def main():
    ...
    for documentService, extensions in extensionProperties.items():
        if documentService not in appDocumentServices.values():
            # Inconsistency found.
            ...
            for extension, flags in extensions.items():
                if "IMPORT" in flags and "EXPORT" in flags:
                    ...
                    action = "view"
                if documentService not in proposed:
                    proposed[documentService] = {}
                proposed[documentService][extension] = action
