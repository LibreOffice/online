...
{
    static Poco::AutoPtr<Poco::Crypto::RSAKey> pKey = []() -> Poco::Crypto::RSAKey* {
        try
        {
...
std::string GetWopiProofKey()
{
    static const std::string publicKey = []{
        ...
        if (auto pos = sKey.find(sBegin); pos != std::string::npos)
            sKey = sKey.substr(pos + sBegin.length());
        if (auto pos = sKey.find(sEnd); pos != std::string::npos)
