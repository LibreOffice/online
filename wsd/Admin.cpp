...
void Admin::pollingThread()
{
    ...
    while (!_stop && !TerminationFlag && !ShutdownRequestFlag)
    {
        ...
        if (memWait <= 0)
        {
            ...
            // If our total memory consumption is above limit, enter hard mode
            enterHardModeIfOOM(totalMem);
        }
...
void Admin::enterHardModeIfOOM(size_t totalMem)
{
    LOG_TRC("Total memory we are consuming (in kB): " << totalMem);
    // Trigger hardmode wh en we are consuming too much memory (as configured by sysadmin)
    const auto hardModeLimit = LOOLWSD::getConfigValue<double>("hardmode", static_cast<double>(80.0));
    LOG_TRC("HardMode Limit configured : " << hardModeLimit);
    float memToFreePercentage = 0;
    if ( (memToFreePercentage = (totalMem/static_cast<double>(_totalSysMem)) - hardModeLimit/100.) > 0.0 )
    {
        ...
        std::list<DocBasicInfo> docList = _model.getDocumentsBasicInfo();
        ...
        LOG_TRC("Checking unsaved documents in document list, length: " << docList.size());
        // Start killing unsaved documents too
        docIt = docList.begin();
        while (docIt != docList.end() && memToFree > 0)
        {
            LOG_TRC("Document: DocKey[" << docIt->_docKey << "], Idletime[" << docIt->_idleTime << "],"
                    << " Saved: [" << docIt->_saved << "], Mem: [" << docIt->_mem << "].");
            LOG_DBG("OOM: Killing unsaved document with DocKey " << docIt->_docKey);
            LOOLWSD::closeDocument(docIt->_docKey, "oom");
            memToFree -= docIt->_mem;
            docList.erase(docIt);
        }
