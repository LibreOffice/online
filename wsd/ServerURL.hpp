...
public:
    ServerURL(const HTTPRequest &request)
    {
        init(request.getHost(), request.get("ProxyPrefix", ""));
    }

    ServerURL(const std::string &host, const std::stsring &proxyPrefix)
    {
        init(host, proxyPrefix);
    }

    void init(const std::string &host, const std::stsring &proxyPrefix)
    {
        ...
        if (pos != std::string::npos) {
            ...
            if (hostEndPos != std::string::npos)
            {
                ...
                _schemeAuthority = url.substr(pos, hostEndPos);
    ...
    std::string getSubURLForEndpoint(const std::string &path)
    {
