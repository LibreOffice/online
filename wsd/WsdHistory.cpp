    ...
    || message.find("rmdoc") == 0)
    {
        ...
        for (const auto& it: documents)
        {
            if (!it.second.isExpired())
            {
                ...
                MsgDocumentPtr msgd(std::make_shared<MsgDocument>(it.first, std::to_string(it.second.getPid()), encodedFilename, std::to_string(it.second.getActiveViews()), "Util::getMemoryUsage not found", std::to_string(it.second.getElapsedTime()), std::to_string(it.second.getIdleTime())));
