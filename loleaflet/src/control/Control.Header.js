...
L.Control.Header = L.Control.extend({
	...
	clearSelection: function () {
		...
		for (var i=start; i<end; i++) {
			if (i === this._current) {
				...
				this.select(this._tickMap.getGap(i));
			} else {
	...
	_onMouseMove: function (e) {
		...
		if (mouseOverIndex && (!this._mouseOverEntry || mouseOverIndex !== this._mouseOverEntry.index)) {
			...
			this._mouseOverEntry = this._tickMap.getGap(mouseOverIndex);
			this.drawHeaderEntry(this._mouseOverEntry, true);
 ...
 *
 * Data abotu gaps is 1-indexed: The 1st gap (row 1 / column A) is 1, and spans
