		...
		vex.dialog.open({
			...
			beforeClose: function () {
				if (!calledFromMenu) {
					...
					WSDVerCookie += '; max-age=31536000; SameSite=None; Secure; path=' + cookiePath;
	...
	showWelcomeDialog: function(calledFromMenu) {
		...
		$.get(welcomeLocation)
			...
			.fail(function() {
				...
				if (window.socketProxy)
					cookiePath = window.host + window.serviceRoot + cookiePath;
				var welcomeDisabledCookie = 'WSDWelcomeDisabled=true; max-age=86400; SameSite=None; Secure; path=' + cookiePath;
