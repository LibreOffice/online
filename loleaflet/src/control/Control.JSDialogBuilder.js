...
L.Control.JSDialogBuilder = L.Control.extend({
	...
	_borderControlItem: function(parentContainer, data, builder, i) {
		...
		$(div).click(function () {
			...
			var item = L.LOUtil.findItemWithAttributeRecursive(data.parent, 'id', 'borderlinecolor');
			if (item)
	...
	_borderControl: function(parentContainer, data, builder) {
		...
		console.debug(data);
		if (data.text) {
			bordercontrollabel.innerHTML = builder._cleanText(data.text);
		}
		else { /*this whole if condition needs to go and we should get the text from data (but from where?)*/
			bordercontrollabel.innerHTML = _('Line Arrangement');
		}
