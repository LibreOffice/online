	...
	onUpdatePermission: function(e) {
		...
		if (toolbar) {
			...
			for (var idx in items) {
				...
				if (e.perm === 'edit') {
					var unoCmd = this.map.getDocType() === 'spreadsheet' ? items[idx].unosheet : window.getUNOCommand(items[idx].uno);
					var keepDisabled = this.map['stateChangeHandler'].getItemValue(unoCmd) === 'disabled';
