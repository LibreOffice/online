...
L.Map = L.Evented.extend({
	...
	isEditingAnnotation: function() {
		...
		{
			annotations = this._docLayer._annotations[this._selectedPart];
		...
		else if (this._docLayer._docType === 'presentation')
		{
			if (this._docLayer._draft)
				return true;
			annotations = this._docLayer._annotations[this._docLayer._partHashes[this._docLayer._selectedPart]];
			for (key in annotations) {
				if (annotations[key].isEdit())
					return true;
			}
			return false;
		}
