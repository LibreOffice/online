...
/* libreoffice-specific code follows */
...
L.Map.include({
	...
	// TODO: Get functionality out of initialize() and into addInitHook() calls
	initialize: function (id, options) { // (HTMLElement or String, Object)
		options = L.setOptions(this, options);
	...
	// Overwritten Leaflet method follows
	// This implementation does *not* enable/disable the 'moveend' event handlers,
	// thus removing all run-time calls to _panInsideMaxBounds
	setMaxBounds: function (bounds) {
		bounds = L.latLngBounds(bounds);
	...
	// Overwritten Leaflet method follows
	// This implementation does not reset this._initialCenter
	invalidateSize: function (options) {
		if (!this._loaded) { return this; }
	...
	// Overwritten Leaflet method follows
	// This implementation does *not* check for this._initialCenter.
	getCenter: function () { // (Boolean) -> LatLng
		this._checkIfLoaded();
		return this.layerPointToLatLng(this._getCenterLayerPoint());
	},
	...
	// Overwritten Leaflet method follows
	// TODO: Move functionality to a subclass of L.CRS.Simple
	getScaleZoom: function (scale, fromZoom) {
		fromZoom = fromZoom === undefined ? this._zoom : fromZoom;
	...
	// Overwritten Leaflet method follows
	// This implementation does *not* set this._initialCenter.
	_resetView: function (center, zoom, preserveMapOffset, afterZoomAnim) {
	...
	// This implementation:
	// - Sets this._mouseOut
	// - Removes contextmenu and keypress, and adds dragover, drop, trplclick and qdrplclick.
	// - Adds a handler for the resiz detector
	// - Adds handlers for blur and focus on the current window
	_initEvents: function (remove) {
		if (!L.DomEvent) { return; }
	...
	_handleDOMEvent: function (e) {
		...
		// Calling from some other place with no reaMostlyl 'click' event doesn't work.
	...
	// Overwritten Leaflet method follows
	// No changes in code, only comments
	_rebound: function (left, right) {
		return left + right > 0 ?
			Math.round(left - right) / 2 :
			Math.max(0, Math.ceil(left)) - Math.max(0, Math.floor(right));
