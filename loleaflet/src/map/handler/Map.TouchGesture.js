...
L.Map.TouchGesture = L.Handler.extend({
	...
	_onSwipe: function (e) {
		...
		var evt = this._constructFakeEvent({},'mousedown');
		evt.clientX = e.center.x;
		evt.clientY = e.center.y;
		evt._startPos = this._newPos;
	...
	_autoscroll: function() {
		...
		if (delta.x > 0.2 || delta.x < -0.2 || delta.y > 0.2 || delta.y < -0.2) {
			...
			e._startPoint = this._startSwipePoint;
			e._startPos = this._newPos;
