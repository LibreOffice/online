...
L.TileLayer.include({
	...
	_createMarker: function(markerType, entry, baseline) {
		...
		if (markerType === 'column')
			marker._pointTwips = new L.Point(this._tablePositionColumnOffset + marker._position, baseline);
		else
			marker._pointTwips = new L.Point(baseline, this._tablePositionRowOffset + marker._position);
	...
	_onTableSelectedMsg: function (textMsg) {
		...
		if (message.rows && message.rows.entries.length > 0 && message.columns && message.columns.entries.length > 0) {
			...
			entry = { type: 'left', position: message.rows.left, index: 0 };
