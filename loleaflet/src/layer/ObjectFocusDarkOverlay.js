...
L.ObjectFocusDarkOverlay = L.ObjectFocusDarkOverlay.extend({
	...
	_onStateChanged: function(args) {
		...
		var fullWidth = 10000;
		var fullHeight = 10000;

		this._createPart('dark-top', 0, 0, fullWidth + 'px', (args.y + 2) + 'px');
		this._createPart('dark-left', 0, args.y + 'px', args.x + 'px', (args.h + 2) + 'px');
		this._createPart('dark-right', args.x + args.w + 'px', args.y + 'px', fullWidth + 'px', (args.h + 2) + 'px');
		this._createPart('dark-bottom', 0, (args.y + args.h - 1) + 'px', fullWidth + 'px', fullHeight + 'px');
	}
});
