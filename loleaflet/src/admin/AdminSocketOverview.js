	...
	onSocketMessage: function(e) {
		...
		if (textMsg.startsWith('documents')) {
			...
			for (var i = 0; i < documents.length; i++) {
				...
				sDocTime = docProps[4];
				sDocIdle = docProps[5];
				jsonStart = documents[i].indexOf('{');
				userJson = documents[i].substr(jsonStart).trim();
				...
				userListJson = JSON.parse(userJson, function(key, value) {
					// key   - username
					// value - sessionID
