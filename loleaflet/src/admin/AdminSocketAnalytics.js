	...
	_updateAverage: function(option, reset) {
		...
		if (reset) {
			for (i = 0; i <= this._netStatsSize - this._netAvgSize; i++) {
				...
				this._sentAvgStats[i].value = tempSum;
			}
		}
		else {
			tempSum = 0;
			for (i = 1; i <= this._netAvgSize; i++) {
				tempSum += data[this._netStatsSize - i].value;
			}
			tempSum /= this._netAvgSize;
			console.log(tempSum);

			if (option === 'sent')
				this._addNewData(this._sentAvgStats, tempSum, 'sent_avg');
			else if (option === 'recv')
				this._addNewData(this._recvAvgStats, tempSum, 'recv_avg');
		}
	},
