...
function onClick(e, id, item, subItem) {
	...
	else if (id.startsWith('menu-wrap:wrap-'))
	{
		...
		e.done(function () {
			var selected = w2ui['toolbar-down'].get('StateTableCellMenu').selected;
			if (subItem.id === '1') { // 'None' was clicked, remove all other options
				selected = ['1'];
				// Set check mark on 'None' on UI, too
				w2ui['toolbar-down'].get('StateTableCellMenu').selected = ['1'];
			}
			else { // Something else was clicked, remove the 'None' option from the array
				var index = selected.indexOf('1');
				if (index > -1) {
