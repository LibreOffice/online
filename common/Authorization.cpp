        ...
        case Type::Header:
        {
            ...
            for (const auto& token : tokens)
            {
                ...
                if (separator != std::string::npos)
                {
                    ...
                    {
                        size_t header_length = headerEnd - headerStart + 1;
                        size_t value_length = valueEnd - valueStart + 1;

                        request.set(token.substr(headerStart, header_length), token.substr(valueStart, value_length));
                    }
                }
            }
            break;
