...
bool handleSetrlimitCommand(const std::vector<std::string>& tokens)
{
    if (tokens.size() == 3 && tokens[0] == "setconfig")
    {
        ...
        {
            lim = std::stoi(tokens[2]) * 1024 * 1024;
            resource = RLIMIT_AS;
            resourceText = "RLIMIT_AS";
            unitText = "bytes";
        }
        else if (tokens[1] == "limit_stack_mem_kb")
        {
            lim = std::stoi(tokens[2]) * 1024;
            resource = RLIMIT_STACK;
            resourceText = "RLIMIT_STACK";
            unitText = "bytes";
        }
        else if (tokens[1] == "limit_file_size_mb")
        {
            ...
            resourceText = "RLIMIT_FSIZE";
            unitText = "bytes";
        }
        else if (tokens[1] == "limit_num_open_files")
        {
            lim = std::stoi(tokens[2]);
            resource = RLIMIT_NOFILE;
            resourceText = "RLIMIT_NOFILE";
            unitText = "files";
        }
        if (lim <= 0)
            lim = RLIM_INFINITY;
