                ...
                if (ws->poll(waitTime, Socket::SELECT_READ))
                {
                    ...
                    else
                    {
                        ...
                        else if (tokens.count() == 1 && tokens[0] == "documents")
                        {
                            ...
                            std::string response = "documents " + admin->getDocuments();
...
public:
    ...
    HTTPRequestHandler* createRequestHandler(const HTTPServerRequest& request) override
    {
        ...
        logger << Log::end;
        return new AdminRequestHandler(admin);
    }
...
void Admin::handleInput(std::string& message)
{
    ...
    if (tokens.count() > 2 && tokens[0] == "document")
    {
        ...
        Log::info() << "adding document " << Log::end;
        model.addDocument(std::stoi(pid), url);
    }
}
...
void Admin::run()
{
    ...
    // Start listening for broker data changes
    std::string message;
    ...
    static const std::string thread_name = "adm_processor";
#ifdef __linux
    if (prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(thread_name.c_str()), 0, 0, 0) != 0)
        Log::error("Cannot set thread name to " + thread_name + ".");
#endif
    ...
    while (!TerminationFlag)
    {
        if (start == end)
        {
            ...
            else if (pollPipeBroker.revents & (POLLIN | POLLPRI))
            {
            ...
            else if (pollPipeBroker.revents & (POLLERR | POLLHUP))
            {
