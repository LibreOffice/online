...
private:
    ...
    static bool handleGetWOPIDiscovery(HTTPServerRequest& request, HTTPServerResponse& response)
    {
        ...
        const std::string uriValue = (SSLEnabled ? "https://" : "http://") +
...
{
    return (SSLEnabled) ? new SecureServerSocket(nClientPortNumber)
...
std::string lcl_getLaunchURI()
{
    ...
    aLaunchURI += ((SSLEnabled) ? "https://" : "http://");
...
#if ENABLE_SSL
    SSLEnabled = getConfigValue<bool>(conf, "ssl.enable", true);
#endif
    if (SSLEnabled)
    {
