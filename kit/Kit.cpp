...
private:
            ...
            unsigned char* to, int to_width, int to_height)
    {
        for (int to_y = from_offset_y, from_y = 0; (to_y < to_height) && (from_y < from_height) ; ++to_y, ++from_y)
            for (int to_x = from_offset_x, from_x = 0; (to_x < to_width) && (from_x < from_width); ++to_x, ++from_x)
            {
                ...
                unsigned char out_r = out_a > 0.0? (src_r * src_a + dst_r * dst_a * (1.0 - src_a)) / out_a: 0;
                unsigned char out_g = out_a > 0.0? (src_g * src_a + dst_g * dst_a * (1.0 - src_a)) / out_a: 0;
                unsigned char out_b = out_a > 0.0? (src_b * src_a + dst_b * dst_a * (1.0 - src_a)) / out_a: 0;
    ...
    const unsigned char* getPixmap(int width, int height)
    {
        ...
        // Create the blurred background: First create a copy of the text that
        // has white (ffffff) in RGB, but alpha as the 'text'
        _pixmap = static_cast<unsigned char*>(malloc(pixel_count));
        unsigned char* from = text;
        unsigned char* to = _pixmap;
        while (to < _pixmap + pixel_count)
        {
            *to++ = 0xff;
            *to++ = 0xff;
            *to++ = 0xff;
            ...
            for (int y = r; y < height - r; ++y)
            {
                for (int x = r; x < width - r; ++x)
                {
                    ...
                    for (int ky = y - r; ky <= y + r; ++ky)
                    {
                        ...
                        {
                            t += _pixmap[4 * (ky * width + kx) + 3];
                        }
                    }

                    // Clamp the result.
                    const double avg = t / weight;
                    buffer[4 * (y * width + x) + 3] = static_cast<unsigned char>(avg <= 255.0 ? avg : 255);
                }
            }

            std::swap(buffer, _pixmap);
        }
        ...
        for (unsigned char* p = _pixmap; p < _pixmap + pixel_count; p += 4)
        {
            p[3] = static_cast<unsigned char>(_alphaLevel * p[3]);
        }
