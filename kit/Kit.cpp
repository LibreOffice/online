...
// In the iOS app we can have several documents opne in the app process at the same time, thus
// several lokit_main() runnins at the same time. We want just one LO main loop, though, so we start
// it separately in its own thread.
