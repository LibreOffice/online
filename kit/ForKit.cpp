                                ...
                                bool queryVersion = false)
{
    ...
    JailUtil::updateSysTemplate_DynamicFiles(sysTemplate);
...
int main(int argc, char** argv)
{
    ...
    JailUtil::setupSysTemplate_DynamicFiles(sysTemplate);

    // Make the real lo path in the chroot point to the chroot lo/.
    JailUtil::setupSysTemplate_LoSymlink(sysTemplate, loTemplate, loSubPath);

    // Make dev/[u]random point to the writable devices in tmp/dev/.
    JailUtil::setupSysTemplate_RandomDeviceLinks(sysTemplate);
