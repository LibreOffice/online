...
#define MAP_SIZE 20
#define PATH_SIZE                                                                                  \
    1000 // No harm in having it much larger than strictly necessary. Avoids compiler warning.
#define BUFFER_SIZE 9600

static int read_buffer(char* buffer, unsigned size, const char* file, char sep)
{
    int file_desc;
    ...
    for (;;)
    {
        ssize_t number_bytes = read(file_desc, buffer + total_bytes, size - total_bytes);
        if (number_bytes == -1)
        {
            if (errno == EINTR)
                continue;
...
public:
    ...
    void printStats()
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            printf("%cStrings      :%20lld, %lld chars\n", prefixes[i],
                   (addr_t)_strings[i].getCount(), (addr_t)_strings[i].getChars());
        }
    }
    ...
    bool isStringAtOffset(const std::vector<unsigned char>& data, size_t i, uint32_t len,
                          bool isUnicode, std::string& str)
    {
        str = isUnicode ? "U_" : "S_";
        int step = isUnicode ? 2 : 1;
        for (size_t j = i; j < i + len * step && j < data.size(); j += step)
        {
            if (isascii(data[j]) && !iscntrl(data[j]) && (step == 1 || data[j + 1] == 0))
    ...
    bool isCStringAtOffset(const std::vector<unsigned char>& data, size_t i, std::string& str)
    {
        str = "C_";
    ...
    void scanMapsForStrings()
    {
        ...
        for (auto& map : _maps)
        {
            ...
            if (lseek(mem_fd, map.getStart(), SEEK_SET) < 0
                ...
                error(EXIT_FAILURE, errno, "Failed to seek in /proc/%d/mem to %lld", _proc_id,
                      map.getStart());
...
static void dumpDiff(const AddrSpace& space, const std::vector<char>& pageData,
...
static void dumpPages(unsigned proc_id, unsigned parent_id, const char* type,
                      const std::vector<addr_t>& pages, const AddrSpace& space)
{
    ...
    for (addr_t page : pages)
    {
        ...
        if (DumpHex)
        {
            printf("\n%s page: 0x%.8llx (%d/%d) - touched: %d - %s - from %s\n", type, page,
                   (int)++cnt, (int)pages.size(), touched, style, space.findName(page).c_str());
    ...
    printf("\tdirtied bytes touched %6lld per page %.2f\n\n", bytesTouched,
           (double)bytesTouched / pages.size());
}
...
static void dump_unshared(unsigned proc_id, unsigned parent_id, const char* type,
                          const std::vector<addr_t>& vaddrs, const AddrSpace& space)
{
    ...
    if (DumpStrings)
    {
        ...
        {
            if (DumpAll || unShared.find((addr.first & ~0x1fff)) != unShared.end())
...
static void total_smaps(unsigned proc_id, unsigned parent_id, const char* file, const char* cmdline)
{
    FILE* file_pointer;
...
int main(int argc, char** argv)
{
    ...
    if (help)
    {
        ...
        fprintf(stderr,
                "    --hex           Hex dump relevant page contents and diff to parent process\n");
        fprintf(stderr, "    --strings       Print all detected strings\n");
        fprintf(stderr, "    --all           Hex dump all writable pages whether touched or not\n");
        fprintf(stderr,
                "    --width <bytes> Define width of hex dump in bytes, rounded to a power of 2\n");
        return 0;
    }
    ...
    while ((dir_proc = readdir(root_proc)))
    {
        ...
        if (*dir_proc->d_name > '0' && *dir_proc->d_name <= '9')
        {
            ...
            if (read_buffer(cmdline, sizeof(cmdline), path_proc, ' ')
                && (forPid == pid_proc
                    || (forPid == 0 && strstr(cmdline, appOrPid) && !strstr(cmdline, argv[0]))))
            {
