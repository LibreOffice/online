...
namespace helpers
{
    ...
    inline size_t timeSinceTestStartMs()
    {
        return std::chrono::duration_cast<std::chrono::milliseconds>(
                   std::chrono::steady_clock::now() - getTestStartTime())
            .count();
    }
    ...
    inline void sendTextFrame(const std::shared_ptr<LOOLWebSocket>& socket,
                               ...
                               != Poco::Net::WebSocket::FRAME_OP_CLOSE)
                    {
                        if (LOOLProtocol::matchPrefix(prefix, message))
                        {
                            TST_LOG("[" << prefix << "] Matched "
                                        << LOOLProtocol::getAbbreviatedFrameDump(response.data(),
                                                                                 bytes, flags));
                            return response;
                        }
                    }
    ...
    inline void parseDocSize(const std::string& message, const std::string& type, int& part,
                             int& parts, int& width, int& height, int& viewid)
    {
    ...
    inline void sendKeyEvent(std::shared_ptr<LOOLWebSocket>& socket, const char* type, int chr,
