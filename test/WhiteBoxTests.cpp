...
void WhiteBoxTests::testSplitting()
{
    ...
    // Split last, remove delim.
    std::tie(first, second) = Util::splitLast(
        std::string("/owncloud/index.php/apps/richdocuments/wopi/files/13_ocgdpzbkm39u"), '/',
        true);
...
void WhiteBoxTests::testTokenizer()
{
    ...
    tokens = LOOLProtocol::tokenize("tile part=0 width=256 height=256 tileposx=0 tileposy=0 "
...
{
    CPPUNIT_ASSERT_EQUAL(std::string("zesz one zwo flee"),
                         Util::replace("test one two flee", "t", "z"));
    CPPUNIT_ASSERT_EQUAL(std::string("testt one two flee"),
                         Util::replace("test one two flee", "tes", "test"));
    CPPUNIT_ASSERT_EQUAL(std::string("testest one two flee"),
                         Util::replace("test one two flee", "tes", "testes"));
    CPPUNIT_ASSERT_EQUAL(std::string("tete one two flee"),
                         Util::replace("tettet one two flee", "tet", "te"));
    CPPUNIT_ASSERT_EQUAL(std::string("t one two flee"),
                         Util::replace("test one two flee", "tes", ""));
    CPPUNIT_ASSERT_EQUAL(std::string("test one two flee"),
                         Util::replace("test one two flee", "", "X"));
}
...
public:
    ...
    bool onLoad(const std::string& /*sessionId*/, const std::string& /*jailedFilePath*/,
                ...
                const std::string& /*lang*/, const std::string& /*watermarkText*/) override
    {
        return false;
    }
    ...
    std::shared_ptr<lok::Office> getLOKit() override { return nullptr; }

    std::shared_ptr<lok::Document> getLOKitDocument() override { return nullptr; }

    void notifyViewInfo() override {}

    void updateEditorSpeeds(int, int) override {}

    int getEditorId() override { return -1; }

    std::map<int, UserInfo> getViewInfo() override { return {}; }

    std::mutex& getMutex() override { return _mutex; }

    std::mutex& getDocumentMutex() override { return _mutex; }

    std::string getObfuscatedFileId() override { return std::string(); }
...
{
    // these intersect
    CPPUNIT_ASSERT(TileDesc::rectanglesIntersect(1000, 1000, 2000, 1000, 2000, 1000, 2000, 1000));
    CPPUNIT_ASSERT(TileDesc::rectanglesIntersect(2000, 1000, 2000, 1000, 1000, 1000, 2000, 1000));

    CPPUNIT_ASSERT(TileDesc::rectanglesIntersect(1000, 1000, 2000, 1000, 3000, 2000, 1000, 1000));
    CPPUNIT_ASSERT(TileDesc::rectanglesIntersect(3000, 2000, 1000, 1000, 1000, 1000, 2000, 1000));

    // these don't
    CPPUNIT_ASSERT(!TileDesc::rectanglesIntersect(1000, 1000, 2000, 1000, 2000, 3000, 2000, 1000));
    CPPUNIT_ASSERT(!TileDesc::rectanglesIntersect(2000, 3000, 2000, 1000, 1000, 1000, 2000, 1000));

    CPPUNIT_ASSERT(!TileDesc::rectanglesIntersect(1000, 1000, 2000, 1000, 2000, 3000, 1000, 1000));
    CPPUNIT_ASSERT(!TileDesc::rectanglesIntersect(2000, 3000, 1000, 1000, 1000, 1000, 2000, 1000));
}
...
void WhiteBoxTests::testAuthorization()
{
    ...
    Authorization auth4(Authorization::Type::Header,
    ...
    Authorization auth5(Authorization::Type::Header,
                        "  Authorization: Basic huh== \n\r X-Something-More:   else  \n\r");
...
void WhiteBoxTests::testJson()
{
    static const char* testString
        = "{\"BaseFileName\":\"SomeFile.pdf\",\"DisableCopy\":true,\"DisableExport\":true,"
          "\"DisableInactiveMessages\":true,\"DisablePrint\":true,\"EnableOwnerTermination\":true,"
          "\"HideExportOption\":true,\"HidePrintOption\":true,\"OwnerId\":\"id@owner.com\","
          "\"PostMessageOrigin\":\"*\",\"Size\":193551,\"UserCanWrite\":true,\"UserFriendlyName\":"
...
void WhiteBoxTests::testAnonymization()
{
    ...
    static const std::string fileUrl
        = "http://localhost/owncloud/index.php/apps/richdocuments/wopi/files/736_ocgdpzbkm39u/"
          "secret.odt?access_token=Hn0zttjbwkvGWb5BHbDa5ArgTykJAyBl&access_token_ttl=0&permission="
          "edit";
