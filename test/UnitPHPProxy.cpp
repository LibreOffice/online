...
{
public:
    const std::string _proxyPrefix = "ProxyPrefix: http://localhost:" + std::to_string(_PORT_) + "/test.php?req=\n";

    void replaceRequest(std::vector<char>& message)
    {
        // First line includes the file request. We will remove proxy prefix && get real request.
        ...
        std::vector<char>::iterator equalSign = std::find(firstSpace + 1, firstLineEnd, '='); // Position of = sign.
    ...
    void handleRegularSocket(std::shared_ptr<StreamSocket> socket)
    {
        ...
        //LOG_ERR("TEST PROXY:\n" << std::string(socket->getInBuffer().data()));
