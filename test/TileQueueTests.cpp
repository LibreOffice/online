...
void TileQueueTests::testSenderQueue()
{
    ...
    CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(0UL), queue.size());
    ...
    CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(3UL), queue.size());

    CPPUNIT_ASSERT_EQUAL(true, queue.dequeue(item));
    CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(2UL), queue.size());
    CPPUNIT_ASSERT_EQUAL(messages[0], std::string(item->data().data(), item->data().size()));

    CPPUNIT_ASSERT_EQUAL(true, queue.dequeue(item));
    CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(1UL), queue.size());
