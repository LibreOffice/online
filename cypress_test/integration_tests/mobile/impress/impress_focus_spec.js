	...
	it('Select text box, no editing', function() {
		...
		cy.get('#document-container')
			.then(function(items) {
				...
				var posX = items[0].getBoundingClientRect().left + items[0].getBoundingClientRect().width / 4;
				var posY = items[0].getBoundingClientRect().top + items[0].getBoundingClientRect().height / 4;
	...
	it('Single- and Double-click to edit', function() {
		...
		cy.get('.leaflet-marker-icon')
			.then(function(marker) {
				...
				var posX = marker[0].getBoundingClientRect().right +
					(marker[1].getBoundingClientRect().left - marker[0].getBoundingClientRect().right) / 2;
				var posY = marker[0].getBoundingClientRect().top - marker[0].getBoundingClientRect().height;
