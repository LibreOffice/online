...
describe('Calc focus tests', function() {
	...
	it('Formula-bar focus', function() {
		...
		cy.get('textarea.clipboard').type('{enter}').wait(500);
		...
		// Accept changes.
		cy.get('textarea.clipboard').type('{enter}').wait(helper.delayForEventsMs);
		...
		// Validate.
		cy.get('textarea.clipboard').type('{ctrl}a').wait(helper.delayForEventsMs);
