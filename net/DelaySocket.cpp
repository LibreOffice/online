...
class DelaySocket : public Socket
{
    ...
    public:
        ...
        {
            _sendTime = std::chrono::steady_clock::now() + std::chrono::milliseconds(delayMs);
        }
...
public:
    ...
    void setDestination(const std::shared_ptr<DelaySocket>& dest) { _dest = dest; }
    ...
    int getPollEvents(std::chrono::steady_clock::time_point now, int& timeoutMaxMs) override
    {
        if (_chunks.size() > 0)
        {
            int remainingMs = std::chrono::duration_cast<std::chrono::milliseconds>(
                                  (*_chunks.begin())->getSendTime() - now)
                                  .count();
            if (remainingMs < timeoutMaxMs)
                DELAY_LOG("#" << getFD() << " reset timeout max to " << remainingMs << "ms from "
                              << timeoutMaxMs << "ms\n");
            timeoutMaxMs = std::min(timeoutMaxMs, remainingMs);
        }

        if (_chunks.size() > 0 && now > (*_chunks.begin())->getSendTime())
            return POLLIN | POLLOUT;
        else
            return POLLIN;
    }

    void pushCloseChunk() { _chunks.push_back(std::make_shared<WriteChunk>(_delayMs)); }
                    ...
                    int events) override
    {
        if (_state == ReadWrite && (events & POLLIN))
        {
            ...
            {
                DELAY_LOG("#" << getFD() << " error : " << Util::symbolicErrno(errno) << ": "
                              << strerror(errno) << "\n");
        ...
        else // Write if we have delayed enough.
        {
            ...
            if (std::chrono::duration_cast<std::chrono::milliseconds>(now - chunk->getSendTime())
                    .count()
                >= 0)
            {
                ...
                else
                {
                    ...
                    if (len < 0)
                    {
                        ...
                        {
                            DELAY_LOG("#" << getFD() << " failed onwards write " << len
                                          << "bytes of " << chunk->getData().size()
                                          << " queue: " << _chunks.size()
                                          << " error: " << Util::symbolicErrno(errno) << ": "
                                          << strerror(errno) << "\n");
                            changeState(Closed);
                        }
                    }
                    else
                    {
                        DELAY_LOG("#" << getFD() << " written onwards " << len << "bytes of "
                                      << chunk->getData().size() << " queue: " << _chunks.size()
                                      << "\n");
                        if (len > 0)
