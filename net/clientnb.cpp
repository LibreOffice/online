...
public:
    ...
    std::shared_ptr<WebSocket> getWebSocket()
    {
        ...
        return std::shared_ptr<WebSocket>(new WebSocket(*_session, request, response));
    }
};
...
struct Client : public Poco::Util::Application
{
    ...
    void testWebsocketPingPong()
    {
        ...
        ws->sendFrame(&send[0], send.length(), WebSocket::SendFlags::FRAME_TEXT);
...
public:
    int main(const std::vector<std::string>& args) override
    {
        ...
        if (EnableHttps)
        {
            ...
            Poco::Net::SSLManager::instance().initializeClient(nullptr, invalidCertHandler,
                                                               sslContext);
        }
